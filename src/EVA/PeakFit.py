...
class PeakFit(QWidget):
    ...
    def PlotAnalysisSpectra(self, title_lab):
        ...
        return PlotWidget.PlotWidget(self.fig_ana, self.axs_ana, self.plt_ana)
    ...
    def fit_spectra(self):
        ...
        if self.tab1.table_clickpeaks.item(0, 0) is None:
            ...
            #plot results
            self.data_plot.plt.plot(datax, multiGaussianFunc(datax, *popt), label="gaussian")
            self.data_plot.plt.axvline(EMin, color='red', linestyle='--')
            self.data_plot.plt.axvline(EMax, color='red', linestyle='--')
            self.data_plot.plt.legend()
            self.data_plot.draw()
    ...
    def fit_spectra_lmfit(self):
        ...
        try:
            self.ln.remove()
            self.lnr.remove()
            self.ln, = self.data_plot.plt.plot(datax, result.best_fit, label="best fit")
            self.lnr, = self.data_plot.plt.plot(datax, datay - result.best_fit, label='Residual')
        except:
            self.ln, = self.data_plot.plt.plot(datax, result.best_fit, label="best fit")
            self.lnr, = self.data_plot.plt.plot(datax, datay - result.best_fit, label='Residual')
