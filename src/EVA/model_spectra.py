...
# TODO replace this with new detector enum after merge
detector_indices = {
    "GE1": 0,
    "GE2": 1,
    "GE3": 2,
...
def get_closest_x(x, y, target_x):
    diff = abs(x - target_x)
    index = diff.argmin()
    return y[index]

def get_model(elements, proportions, detectors, e_range=None,
    ...
    # TODO: Update this when new database handling is merged into main
    ...
    for j, det in enumerate(detectors):
        ...
        for i, element in enumerate(elements):
            ...
            detector_energy_res = energy_res[detector_indices[det]]

            print(f"Detector: {detector_energy_res[0]} \nc = {detector_energy_res[1]} \nm = {detector_energy_res[2]}")
            ...
            if show_primary:
                for trans in prim_trans:
                    ...
                    if e_res_model == "linear":
                        sigma = line(mean, detector_energy_res[1], detector_energy_res[2]) / (2*np.sqrt(2*np.log(2)))
                    elif e_res_model == "quadratic":
                        sigma = quadratic(mean, detector_energy_res[1], detector_energy_res[2], detector_energy_res[3]) / (2*np.sqrt(2*np.log(2)))
                    else:
                        raise ValueError("Invalid energy resolution model")

                    all_transitions.append({"name": trans, "E": mean, "sigma": sigma, "weights": weights, "type": "primary", "element": element})
            ...
            if show_secondary:
                for trans in sec_trans:
                    ...
                    all_transitions.append({"name": trans, "E": mean, "sigma": sigma, "weights": weights, "type": "secondary", "element": element})
        ...
        for trans in all_transitions:
            trans["curve"] = gaussian(xdata, mean=trans["E"], sigma=trans["sigma"]) * trans["weights"]
            total += trans["curve"]
        ...
        if show_components:
            ax = add_components(ax, xdata, all_transitions, total, notation)

    plt.subplots_adjust(top=0.9, bottom=0.1, left=0.1, right=0.9, hspace=0.45, wspace=0.23)
    return fig, axs

def add_components(ax, xdata, all_trans, total, notation="siegbahn"):
    # load notation dict
    with open("src/EVA/databases/siegbahn_table.json") as file:
        ...
        if np.sum(trans["curve"]) > 0.1:  # Check if the peak is within the viewing range of the plot
            ...
            if notation == "siegbahn":
                ...
                if siegbahn_name:
                    ...
                    ax.text(x=trans["E"], y=peak_height+y_offset, s=rf"{element} ${siegbahn_name}$", fontdict=font, horizontalalignment="center")
                ...
                ax.text(x=trans["E"], y=peak_height+y_offset, s=f"{element} {spec_name}", fontdict=font, horizontalalignment="center")
                ...
                ax.text(x=trans["E"], y=peak_height+y_offset, s=f"{element} {iupac_name}", fontdict=font, horizontalalignment="center")
    return ax